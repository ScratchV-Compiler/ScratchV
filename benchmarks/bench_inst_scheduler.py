...
def bench_schedule(
    ...
    return {
        ...
        "model": stats["model"],
        "metrics_before": stats["metrics_before"],
        "metrics_after": stats["metrics_after"],
...
def _markdown(records: list[dict]) -> str:
    lines = [
        ...
        "",
        "固定随机种子生成合法物理寄存器指令，单独统计优化器耗时和 LLVM MCA 局部周期收益（进程内缓存重复输入）。",
...
def main() -> None:
    ...
    records = []
    print(
        "Synthetic inputs; LLVM MCA static estimates, not measured hardware cycles"
    )
    print(
