...
def run_case(
    case_path: Path, *, static_only: bool = False, repeats: int = 3, llvm_mca: str | None = None
) -> dict[str, Any]:
    ...
    driver = CompilerDriver(CompilerConfig(schedule=True, schedule_strict=True, llvm_mca=llvm_mca))
    times = []
    ...
    direct = schedule_assembly(source, ScheduleConfig(strict=True, llvm_mca=llvm_mca))
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"| LLVM MCA 局部周期（估算） | {s['original_cycles'] if modeled else 'N/A'} | {s['final_cycles'] if modeled else 'N/A'} | {s['saved_cycles'] if modeled else 'N/A'} |",
        ]
    )
