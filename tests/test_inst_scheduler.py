...
class TestInstructionScheduler:
    ...
    def test_schedule_with_dependency(self):
        ...
        # Propose independent work between a producer and its consumer.
    ...
    def test_estimate_cycles(self):
        ...
        # Multiplication has no early integer bypass: add(3) + mul(3).
        assert cycles == 6
    ...
    def test_priority_computation(self):
        """Verify that priorities are computed from graph depth."""
        insts = [
            SchedInst(0, "lw", ["t0", "0(a0)"], defines={"t0"}, uses={"a0"}),
        ...
        scheduler = InstructionScheduler()
        dag = scheduler.build_dag(insts)
        assert [node.priority for node in dag] == [2, 1]
    ...
    def test_waw_dependency(self):
        ...
        dag = scheduler.build_dag(insts)
        assert len(dag) == 2
        assert dag[1].predecessors == [(dag[0], 0)]
