...
def test_all_readers_must_precede_overwrite():
    nodes = graph("add t1, t0, t2\nsub t3, t0, t4\nli x5, 9")
    assert nodes[2].predecessors == [(nodes[0], 1), (nodes[1], 1)]
...
def test_zero_has_no_register_dependencies_but_load_still_has_memory_effect():
    ...
    assert nodes[2].predecessors == [(nodes[0], 1)]
...
def test_exact_heights_and_load_use_wait():
    ...
    assert result.stats["final_cycles"] == 4
    assert result.stats["original_stalls"] == 1
    assert result.stats["final_stalls"] == 0
    row = result.stats["regions"][0]
    assert row["original_issue_cycles"] == [0, 2, 3, 4]
    assert row["candidate_issue_cycles"] == [0, 1, 2, 3]
...
def test_custom_model_is_shared_and_copied():
    values = {"lw": 4}
    model = ScheduleModel(overrides=values)
    values["lw"] = 1
    result = schedule_assembly(EXAMPLE, ScheduleConfig(model=model, strict=True))
    assert result.stats["original_cycles"] == 7
    assert result.stats["final_cycles"] == 6
    assert result.stats["model_overrides"] == {"lw": 4}
    with pytest.raises(ValueError):
        ScheduleModel(overrides={"add": 0})
...
def test_cycles_in_graph_never_return_partial_output():
    scheduler = InstructionScheduler()
    nodes = graph("li t0, 1\nli t1, 2")
    nodes[0].predecessors.append((nodes[1], 1))
    nodes[1].successors.append((nodes[0], 1))
    nodes[1].predecessors.append((nodes[0], 1))
    nodes[0].successors.append((nodes[1], 1))
