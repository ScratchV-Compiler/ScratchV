...
def test_feature_report_uses_compiler_and_executes_equivalently():
    ...
    before, after = scheduling["original_cycles"], scheduling["final_cycles"]
    assert before > after > 0
    assert scheduling["original_stalls"] == scheduling["final_stalls"] > 0
    assert scheduling["moved_instructions"] == 3
    ...
    assert f"| LLVM MCA 局部周期（估算） | {before} | {after} | {before - after} |" in markdown
...
def test_unmodeled_inputs_are_not_reported_as_zero_cycle_runs(tmp_path, source):
    ...
    assert "| LLVM MCA 局部周期（估算） | N/A | N/A | N/A |" in reports._markdown(report)
