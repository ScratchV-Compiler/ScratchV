...
本测试套件用于验证 ScratchV 编译器在多个 DSL 程序上的编译正确性、模拟结果和性能表现。测试脚本会自动遍历或按条件筛选 `tests/topic06/cases/` 下的 DSL 用例，调用 ScratchV 编译器生成 RISC-V 汇编，再通过真实 TinyFive 适配层执行汇编，并将返回值与用例元数据中的期望值比较。测试结束后会在 `benchmark_reports/topic06/` 生成适合人工查看和 CI 解析的报告。
...
Markdown 和 HTML 报告固定展示相同列，不再根据模式增删列。CI 推荐读取 `benchmark_reports/topic06/report.json`，其顶层固定包含 `schema_version`、`mode`、`generated_at`、`summary` 和 `results`，当前 schema 版本为 `1`。
