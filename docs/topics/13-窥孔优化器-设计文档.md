...
> **状态**：✅ 课题收尾完成 | **验证**：相关测试通过（命令见下文），8 条默认规则
...
- Rule 4（mv 链）：仅当中间寄存器在窗口之后的**局部直线代码**中未再被使用（遇标签则保守视为存活）时才改写；完整 CFG 活跃性分析仍为 TODO。
...
| 类别 | 结果 |
|------|------|
| 全部 `tests/test_asm_peephole*.py` | 相关测试通过，命令如下 |
| Benchmark 相关测试 | 相关测试通过，命令如下 |
| 默认规则数 | **8**（假交换删除已移除） |
...
| before_instructions / after_instructions | 共享 _asm_parser.parse_asm 统计的有效 opcode 数：排除空行、纯标签、注释和汇编 directive；计入 RISC-V 真实指令及伪指令 opcode |
| reduced_instructions / reduction_percent | 有效指令静态节省及比例 |
| rule_matches | PR39 八条默认规则的逐条应用次数 |
| elapsed_ms_* | 优化器耗时统计，仅作同机趋势观察；peephole_off 不计时，JSON null 表示未测量 |
...
原有 DSL 对比脚本会编译 `benchmarks/cases` 中的用例，并可生成 JSON/Markdown 报告：
...
独立的 JSON/HTML 脚本对每个预定义汇编样例使用同一份输入，分别记录 peephole 关闭和开启结果：
...
- benchmark_reports/peephole_compare_html.json：稳定 schema（schema_version=1）的机器可读数据；
- benchmark_reports/peephole_compare.html：遵循现有报告样式的汇总卡片、水平条、各规则应用次数、实际应用规则与样例明细。
- benchmark_reports/peephole_dsl_compare.html：复用 `run_dsl_suite()` 结果的 23 个 DSL 案例 HTML 报告，不包含 synthetic 案例。

每个样例包含 peephole_off、peephole_on、输入 SHA-256、有效指令数、静态节省、规则应用次数和参考耗时；样例表中的“应用规则”表示该案例实际执行的规则列表，不等同于指令数是否减少。peephole_off.elapsed_ms_median 不执行关闭路径计时，以 JSON null 表示未测量；reduction_percent 由有效指令数计算，基线为 0 时定义为 0.0%，避免除零。
...
| 规则应用次数 | 0 | 12 | — |
| 未变化样例（changes == 0） | — | 5 / 14 | — |

该数据是 PR39 默认覆盖套件的本地样例数据，只说明当前样例的静态效果，不是性能保证；耗时因机器和 Python 环境变化，不作为固定阈值。生成的 HTML/JSON 位于被 .gitignore 忽略的 benchmark_reports/，可按上述命令随时重建。
...
| mv 链活跃性 | Rule 4：局部死寄存器才改写；跨基本块/完整 CFG 仍 TODO | 中 |
| `--list-rules` CLI | ✅ 已修复：无需 dummy input；另支持 `--json` / `--json-output` | — |
...
| `benchmarks/compare_peephole.py` | DSL/合成样例 JSON、Markdown 对比 |
| `benchmarks/compare_peephole_html.py` | 固定汇编样例 JSON、HTML 对比 |
| benchmark_reports/peephole_compare.md / .html | 对比报告（运行相应脚本生成） |
...
汇编层窥孔优化已可用：**正确性优先**（假交换不删、溢出不合并、标签不丢），规则与测试齐全；默认覆盖套件的本地样例节省 29.032% 静态指令，不是性能保证；合成规模结果随 fusion_ratio 变化，仅用于同机趋势观察。
...
| 测试 | 相关测试通过，命令如下（含语义等价、CLI 与 benchmark 工具） |
| 集成 | `--peephole-asm` / `CompilerDriver.peephole_asm` |
| 文档 | 教程 / 设计文档 / AI 指南 / JSON + HTML 对比报告 |
| 效果 | 默认覆盖套件本地样例 31→22（-29.032%，不是性能保证）；代表性 codegen 样例 6→3（-50%） |
