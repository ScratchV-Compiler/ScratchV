...
每个样例包含 peephole_off、peephole_on、输入 SHA-256、有效指令数、静态节省、规则命中次数和参考耗时。peephole_off.elapsed_ms_median 不执行关闭路径计时，以 JSON null 表示未测量；reduction_percent 由有效指令数计算，基线为 0 时定义为 0.0%，避免除零。
