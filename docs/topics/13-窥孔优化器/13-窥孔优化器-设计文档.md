...
> **状态**：✅ 课题收尾完成 | **最后验证**：2026-08-01（**83/83** 测试，8 条默认规则）
...
- **课题完成目录（总入口）** → [13-窥孔优化器-完成目录.md](13-窥孔优化器-完成目录.md)  
- **面向新手的课题教程** → [13-窥孔优化器-详细教程.md](13-窥孔优化器-详细教程.md)  
- **面向 AI Agent 的实现指南** → [topic13_asm_peephole_guide.md](../../archive/提案/topic13_asm_peephole_guide.md)
...
- 与共享汇编解析器 `_asm_parser.py` 的统一（技术债，见 §8）
...
| 类别 | 结果 |
|------|------|
| 全部 `tests/test_asm_peephole*.py` | **83 PASSED**（约 1.3s） |
| 默认规则数 | **8**（假交换删除已移除） |
...
fusion_ratio=0.3 时，约 **25% 指令可被合并**（合成数据）。

### 7.7 窥孔优化前后对比（2026-08-01）
...
| 维度 | 说明 |
|------|------|
| **优化前** | `peephole_asm=False` |
| **优化后** | `AsmPeepholeOptimizer` / `--peephole-asm` |
| **指标** | 静态指令数（`inst_counter`） |

#### 结果一：DSL 基准（23 用例）

| 指标 | 优化前 | 优化后 | 变化 |
|------|--------|--------|------|
| 静态指令合计 | 231 | 230 | **-1 (-0.43%)** |
| 有节省的用例 | — | 1 / 23 | 仅 `006_softmax`（mv 链） |
...
完整报告：[`benchmark_reports/peephole_compare.md`](../../benchmark_reports/peephole_compare.md)
...
| 项 | 说明 | 优先级 |
|----|------|--------|
| 解析器重复 | 本模块自建 `AsmLine`/`_parse_asm`，与 `_asm_parser.py` 未统一 | 中 |
...
| `benchmark_reports/peephole_compare.md` | 对比报告 |
...
汇编层窥孔优化已可用：**正确性优先**（假交换不删、溢出不合并、标签不丢），规则与测试齐全；小基准收益有限，冗余密集时约省 25% 静态指令。
...
| 测试 | **83 PASSED**（含语义等价与 CLI） |
| 集成 | `--peephole-asm` / `CompilerDriver.peephole_asm` |
| 文档 | 教程 / 设计文档 / AI 指南 / 对比报告 |
| 效果 | DSL -0.43%；合成 ~-25%；综合样例 -46% |
...
> **课题 13 收尾完成。** 维护入口：[topic13_asm_peephole_guide.md](../../archive/提案/topic13_asm_peephole_guide.md)
