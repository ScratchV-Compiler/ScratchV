...
> 工具解析注记：`spike` / `spike-dasm` / `spike-log-parser` 由
> `spike_sim.resolve_spike_tools()` 统一解析（CLI > 环境变量 >
> `SCRATCHV_SPIKE_HOME` > `PATH` > 常见目录 > legacy 常量），缺失时按
> skip / 告警分层降级（详见课题 24 设计文档）。
