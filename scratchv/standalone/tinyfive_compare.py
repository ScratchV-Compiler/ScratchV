...
def generate_tinyfive_report(
    ...
    lines.append(f"  {'Insns per conv MAC':<30s} {'~7':>15s} {'~12':>16s}  {'1.7x':>8s}")
    lines.append(f"  {'Insns per FC MAC':<30s} {'~5':>15s} {'~11':>16s}  {'2.2x':>8s}")
    lines.append(f"  {'Total dynamic insns':<30s} {'~1.06B':>15s} {'~3.22B':>16s}  {'3.0x':>8s}")
    lines.append(f"  {'Total memory ops':<30s} {'~0.53B':>15s} {'~0.55B':>16s}  {'1.0x':>8s}")
    ...
    lines.append(f"  grows to 3.0x in the full model due to ScratchV's additional:")
