...
def describe(opcode: str, operands: tuple[str, ...]) -> InstructionEffects:
    ...
    def boundary(reason: str, control: str = "none") -> InstructionEffects:
        return InstructionEffects(control=control, barrier_reason=reason)
    ...
    if op in {"call", "tail"}:
        return boundary("call boundary", "call")
    if op.startswith("f"):
        parts = op.split(".")
        stem = parts[0]
