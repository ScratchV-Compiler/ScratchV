...
def run_simulation(asm_file: Path, timeout: float = SIMULATION_TIMEOUT_SEC):
    ...
    try:
        completed = subprocess.run(
            [sys.executable, "-c", code, str(asm_file)],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="ignore",
...
def resolve_condition(cond_text, env):
    ...
    for op in ("==", "!=", "<=", ">=", "<", ">"):
        if op in cond_text:
            lhs_text, rhs_text = [part.strip() for part in cond_text.split(op, 1)]
            lhs = resolve_value(lhs_text, env)
            rhs = resolve_value(rhs_text, env)
            if op == "==":
                return lhs == rhs
            if op == "!=":
                return lhs != rhs
    ...
    return bool(resolve_value(cond_text, env))
...
def execute_block(lines, env, start_idx=0, end_idx=None):
    ...
    while idx < end_idx:
        ...
        if line.startswith("if "):
            ...
            cond_value = resolve_condition(cond_text, env)
...
def generate_report_text_cn(results, passed, failed):
    ...
    if benchmark_mode:
        ...
        lines.extend([
            "| 用例 | 类别 | 状态 | 模拟后端 | 指令数 | 编译耗时(s) | 模拟耗时(s) | 总耗时(s) | 预期输出 | 实际输出 | 输出匹配 | 汇编文件 |\n",
            "|---|---|---|---|---:|---:|---:|---:|---|---|---|---|\n",
        ])
...
def write_report(results, passed, failed):
    REPORT_DIR.mkdir(exist_ok=True)
    write_chart(results)
    REPORT_FILE.write_text(generate_report_text_cn(results, passed, failed), encoding="utf-8")
    html_path = write_html_report_cn(results, passed, failed)
...
def main(argv=None):
    ...
    for dsl_file in dsl_files:
        ...
        reference_time_sec = time.perf_counter() - reference_start
